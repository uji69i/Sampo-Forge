...
import { buildTopologyData } from '@sampo-forge/shared/lib/mihomo/topology-data'
...
export function ConfigTopology({ state }: ConfigTopologyProps) {
  ...
  const topologyData = useMemo(
    () => buildTopologyData(state, { expandChains }),
    [state, expandChains]
  )
    ...
    return (
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">{t('mihomo.topologyTitle')}</CardTitle>
        </CardHeader>

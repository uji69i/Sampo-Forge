...
export function GeoRules({
  ...
  const filteredGeosite = geositeSearch.trim()
    ? geositeList.filter((s) => s.toLowerCase().includes(geositeSearch.toLowerCase()))
    : geositeList
  const filteredGeoip = geoipSearch.trim()
    ? geoipList.filter((s) => s.toLowerCase().includes(geoipSearch.toLowerCase()) || isoToFlag(s).length > 0)
    : geoipList
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 pb-2">
        <div>
          <CardTitle className="text-base">GEOSITE / GEOIP</CardTitle>
          <Badge variant="secondary" className="mt-1">{t('mihomo.rulesSubtitle')}</Badge>
        </div>
      </CardHeader>
      <CardContent className="grid grid-cols-1 gap-6 md:grid-cols-2 pt-0">
            ...
            {filteredGeosite.slice(0, geositeShown).map((name) => (
              <div key={name} className="flex items-center justify-between gap-2">
                ...
                <Select value={rulesGeosite.get(name)?.target ?? 'empty'} onValueChange={(v) => onSetGeositePolicy(name, v === 'empty' ? 'DIRECT' : v)}>
                  <SelectTrigger className="w-[110px] h-8"><SelectValue placeholder="—" /></SelectTrigger>
